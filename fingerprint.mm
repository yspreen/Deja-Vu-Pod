...
- (char *) get_fingerprints: (float *) data data_size:(int)data_size {
  ...
  cv::dft(dst, dst, cv::DFT_COMPLEX_OUTPUT + cv::DFT_ROWS, 0);
  ...
  char name[json.size() + 100];
  snprintf(name, sizeof name, "%s", json.c_str());
  return strdup(name);
}
